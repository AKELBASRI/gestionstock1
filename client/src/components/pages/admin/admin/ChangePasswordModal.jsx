...
function ChangePasswordModal({usernormal,show,handleClose}) {
    ...
    if(validate()){
     ...
     .then(res=>{
         if(res.error){
             ...
             //props.history.push('/');
             toastr.success(`Changement mot de passe bien effectué ${usernormal.nom}   `,'Changement mot de passe Utilisateur',{
                 positionClass:"toast-bottom-left"
             });
  ...
  const validate=()=>{
      ...
      else if(typeof input.password!=="undefined" && typeof input.password2!=="undefined" ) {
    ...
    <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Modification mot de passe du : {usernormal.nom} Matricule : {usernormal.Mle}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group  >
            <Form.Label>Mot de passe</Form.Label>
            <Form.Control onChange={handleChange} type="password" placeholder="Mot de passe" id="password" />
            <div className="text-danger">{errors.password}</div>
         
            
            <Form.Label>Confirmer mot de passe </Form.Label>
            <Form.Control onChange={handleChange}   type="password" placeholder="Confirmer mot de passe" id="password2" />
