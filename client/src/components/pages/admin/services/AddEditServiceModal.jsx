...
function AddEditServiceModal({ CodeSce,show,handleClose}) {
    ...
    const service = useSelector((state) =>CodeSce? state.serviceReducer.find((p)=>p.id===CodeSce):null);
    useEffect(()=>{
            if(service){
                setService(service)
            }else{
    ...
    const AddService=()=>{
       ...
       .then(res=>{
           if(res.error){
               ...
               //props.history.push('/');
               toastr.success(`Le service ${nservice.Libelle}  est crée avec succés `,'Nouveau Service',{
                   positionClass:"toast-bottom-left"
               });
               setService({Libelle:''})
    ...
    return (
           ...
           <Modal.Header closeButton>
             ...
             <Modal.Title>{service ? `Modification  du service : ${service.service_name} ` :'Ajout Service' }</Modal.Title>
           </Modal.Header>
           <Modal.Body>
